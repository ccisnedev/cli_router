...
void main() {
  group('run() dispatches to the matched handler', () {
    test('a successful resolution invokes the route handler with a request', () async {
      CliRequest? captured;
    ...
    test('CliRequest.option returns null when the option was not read', () async {
      CliRequest? captured;
      final router = CliRouter();
      router.cmd(
        'build',
  ...
  group('run() calls onReject for a rejection, never a silent default', () {
    test('onReject receives the CliRejection and its return code is used', () async {
      ...
      noShortcut.cmd('version', (req) async => 0, options: const [], globals: false);
  ...
  group('CliRequest has no flags map and no matchedCommand/help helpers', () {
    test('the request shape exposes only route, params, rest, options', () async {
      CliRequest? captured;
      final router = CliRouter();
      router.cmd('ping', (req) async {
        captured = req;
        return 0;
      }, options: const [], globals: false);
